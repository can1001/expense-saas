...
### 배경
결재자(1차/2차/3차)가 결재 시 해당 세목의 예산 금액과 잔액을 확인할 수 있도록 함.
일반 사용자(작성자)에게는 예산 정보를 표시하지 않음.

### 생성 파일
- `components/approval/BudgetInfoPanel.tsx` - 예산 현황 패널 컴포넌트

### 변경 파일
- `app/api/expenses/[id]/approval/route.ts` - 결재자용 예산 정보 API 추가
- `app/approvals/[id]/page.tsx` - BudgetInfoPanel 통합

### API 변경

**GET** `/api/expenses/[id]/approval?approverName=홍길동`

결재자인 경우 응답에 `budgetInfo` 추가:
