...
- [x] 관리 권한자 홈: 딥그린 사이드바(전역 config, 결재함 뱃지 포함) + 회계 대시보드 렌더링
- [x] 일반 사용자 홈: 기존 HomeClient 그대로 (스냅샷 수준 무변화)
- [x] "자동이체" 표기가 "정기 지출"로 통일 (Header·페이지 타이틀)
- [x] HomeClient의 `/youth-night` 잔여 링크 제거
- [x] 사이드바 하단 사용자 카드(이름·이메일·로그아웃) 동작
