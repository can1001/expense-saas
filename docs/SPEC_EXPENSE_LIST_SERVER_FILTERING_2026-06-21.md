...
| 모바일 UI | **기존 무한 스크롤 유지** — 단, 다음 서버 페이지를 누적 fetch |
| 정렬 범위 | **Expense 직속 컬럼 8개만 서버 이동** — requestDate, applicantName, requestAmount, committee, status, approvedAt, expenseDate, paymentStatus. 예산 3개(budgetCategory, budgetSubcategory, budgetDetail)는 ExpenseItem 1:N 관계 정렬이 Prisma `orderBy`로 불가능 → **정렬 기능 제거**, 컬럼 헤더는 표시. |
| 검색어 debounce | **300ms** |
| 작업 방식 | 새 브랜치 `feat/expense-list-server-filtering` + PR |
