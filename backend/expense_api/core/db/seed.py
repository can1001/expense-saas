...
async def seed_if_needed() -> None:
    ...
    async with async_session_maker() as session:
        ...
        session.add(cat_hr)
        await session.flush()
        sub_meeting = BudgetSubcategory(tenantId=tid, categoryId=cat_admin.id, name="회의비", sortOrder=1)
        ...
        await session.flush()
        det_snack = BudgetDetail(tenantId=tid, subcategoryId=sub_meeting.id, name="간식비", sortOrder=1)
        det_tea = BudgetDetail(tenantId=tid, subcategoryId=sub_meeting.id, name="다과비", sortOrder=2)
        det_reg = BudgetDetail(tenantId=tid, subcategoryId=sub_salary.id, name="정규직급여", sortOrder=1)
