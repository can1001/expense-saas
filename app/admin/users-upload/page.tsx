...
export default function UsersUploadPage() {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      a.href = url;
      a.download = `users_template_${new Date().toISOString().split('T')[0]}.xlsx`;
      a.click();
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className={`${SECTION_CARD} mb-6`}>
        ...
        <div className="text-sm text-gray-600 space-y-2">
          <p>Excel 파일을 업로드하여 사용자를 일괄 등록/수정할 수 있습니다.</p>
          <p>
            <strong>열 구조:</strong> userid (아이디), username (이름), role (역할), department (부서), isActive (활성화)
          </p>
          <p>
            <strong>역할 옵션:</strong> 관리자, 재정팀장, 회계, 팀장, 행정간사, 사용자
          </p>
          <p>
            <strong>기본 비밀번호:</strong> 새로 생성된 사용자는 <code className="bg-gray-100 px-1 rounded">chc2026</code> 비밀번호가 설정됩니다.
          </p>
        </div>
      </div>
      ...
      <div className={SECTION_CARD}>
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-xs text-gray-500">
            기존 사용자 데이터가 포함된 Excel 파일을 다운로드합니다. 이 파일을 수정하여 업로드하세요.
          </p>
        </div>
          ...
          <div className="flex gap-6 mt-2">
            <label className="flex items-start cursor-pointer">
              ...
              <span className="text-sm">
                ...
                <br />
                <span className="text-gray-500">기존 사용자 정보 업데이트, 새 사용자 추가</span>
              </span>
            </label>
            <label className="flex items-start cursor-pointer">
              <input
              ...
              />
              <span className="text-sm">
                <strong>추가만 (Append)</strong>
                <br />
                <span className="text-gray-500">새 사용자만 추가, 기존 사용자는 건너뜀</span>
              </span>
            </label>
          </div>
        </div>
          ...
          >
            ...
            {result.data?.summary && (
              <div className="text-sm space-y-1">
                ...
                <p className="text-green-600">생성 예정: {result.data.summary.created}명</p>
                <p className="text-blue-600">업데이트 예정: {result.data.summary.updated}명</p>
                <p className="text-gray-600">건너뜀: {result.data.summary.skipped}명</p>
